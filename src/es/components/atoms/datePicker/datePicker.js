...
class Day {
  constructor(date = new Date(), lang = 'default') {
    ...
    this.week = getCurrentWeekNumber(date);
  }
...
export default class DatePicker extends Shadow() {
  ...
  connectedCallback() {
    ...
    this.toggleButton = this.root.querySelector('.date-toggle');
    this.calendarDropDown = this.root.querySelector('.calendar-dropdown');
  ...
  handleClickOut(e) {
    if (this.visible && (this.root !== e.target)) {
